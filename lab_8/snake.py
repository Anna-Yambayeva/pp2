...
class Snake: # класс змейки
    ...
    def draw(self): # для рисования змейки
        head = self.body[0]
        pygame.draw.rect(screen, colorRED, (head.x * CELL, head.y * CELL, CELL, CELL)) # рисуем голову змейки
        for segment in self.body[1:]:
            pygame.draw.rect(screen, colorYELLOW, (segment.x * CELL, segment.y * CELL, CELL, CELL)) # рисуем тело змейки
    ...
    def check_collision(self, food): # проверяем столкновение с едой
        ...
        if head.x == food.pos.x and head.y == food.pos.y: # если голова змейки совпадает с позицией еды то увеличиваем длину змейки
            ...
            food.generate_random_pos(self) # генерируем новую еду в случайной позиции
            return True  # Вернём True, чтобы увеличить счёт
        return False

class Food: # класс еды
    def __init__(self): # задаем начальные координаты еды
        self.pos = Point(9, 9)

    def draw(self): # для рисования еды
        pygame.draw.rect(screen, colorGREEN, (self.pos.x * CELL, self.pos.y * CELL, CELL, CELL))

    def generate_random_pos(self, snake): # генерируем случайную позицию для еды
        while True: 
